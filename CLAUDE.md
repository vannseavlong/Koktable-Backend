...
On `^0.1.43` (upgraded from `0.1.18` — see `CHANGELOG.md` in the package for the full history). Things worth knowing when bumping this dependency further:

- **`AuthRouterOptions.scopes?: string[]`** (0.1.43) — overrides the OAuth scopes a `createAuthRouter` login redirect requests; defaults to the still-exported `LOGIN_SCOPES` (`openid`, `email`, `profile`, plus the sensitive `spreadsheets`/`drive.file` pair) for backward compatibility. `app.ts`'s three Google routers (mobile, web, admin) now all pass `scopes: IDENTITY_ONLY_SCOPES` (`['openid', 'email', 'profile']`) — none of them need Sheets/Drive access on the *end user's own* grant, since all Sheets/Drive calls in this repo run server-side under the separate admin-owned token from `pnpm db:sync`. Trimming the sensitive scopes off login means Google no longer shows the "hasn't verified this app" interstitial on any of the three login flows. `scopes` must include `'openid'` (throws `ValidationError` at router-creation time otherwise) — unaffected by this change since `IDENTITY_ONLY_SCOPES` keeps it. `pnpm db:sync`'s own OAuth grant (`createOAuthManager`, not `createAuthRouter`) still requests `spreadsheets`/`drive.file` as before — that flow genuinely needs them, so it still shows the warning once per admin-token setup.
